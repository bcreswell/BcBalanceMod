...
import com.megacrit.cardcrawl.powers.ThinkingAheadPower;
...
public class ThinkingAhead extends BcSkillCardBase
{
    ...
    {
        return "Draw "+ getCardCountString(magicNumber)+". NL At the End of this Turn, Pick "+ getCardCountString(getRetainCount())+" to Retain. NL NL Draw 1 less card next turn.";
    }
    //endregion
    
    public void use(AbstractPlayer player, AbstractMonster monster)
    {
        int retainCount = getRetainCount();
        
        addToBot(new DrawCardAction(player, magicNumber));
        addToBot(new BcApplyPowerAction(new RetainCardPower(player, retainCount)));
        addToBot(new BcApplyPowerAction(new ThinkingAheadPower(player, retainCount)));
