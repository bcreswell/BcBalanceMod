...
public abstract class AbstractCard implements Comparable<AbstractCard>
{
    ...
    public AbstractCard(String id, String name, String deprecatedJokeUrl, String imgUrl, int cost, String rawDescription, AbstractCard.CardType type, AbstractCard.CardColor color, AbstractCard.CardRarity rarity, AbstractCard.CardTarget target, DamageInfo.DamageType dType)
    {
        this(id, name, imgUrl, cost, rawDescription, type, color, rarity, target, dType);
    }
    
    public AbstractCard(String id, String name, String imgUrl, int cost, String rawDescription, AbstractCard.CardType type, AbstractCard.CardColor color, AbstractCard.CardRarity rarity, AbstractCard.CardTarget target, DamageInfo.DamageType dType)
    {
        this.chargeCost = -1;
    ...
    {
        titleRenderScale = 1.0f;
        titleToRender = name;
        
        float maxWidth = TITLE_BOX_WIDTH;
        if (cost < -1)
        {
            //unplayable cards
            maxWidth = TITLE_BOX_WIDTH_NO_COST;
        }
        
        while (true)
        {
            FontHelper.cardTitleFont.getData().setScale(titleRenderScale);
            ...
            else
            {
                ...
                titleRenderScale -= .03f;
                //this offset only used by SingleCardViewPopup
                titleXOffset += 1;
            }
    ...
    public void initializeDescription()
    {
        ...
        else
        {
            ...
            for (int var6 = 0; var6 < var5; ++var6)
            {
                ...
                {
                    //remove color letter so it doesn't word wrap prematurely
                    gl.setText(FontHelper.cardDescFont_N, word.substring(2) + sbuilder2);
                }
    ...
    public AbstractCard makeStatEquivalentCopy()
    {
        ...
        card.titleRenderScale = this.titleRenderScale;
        card.titleToRender = this.titleToRender;
        card.titleXOffset = this.titleXOffset;
        card.isEthereal = this.isEthereal;
        card.tempUpgradeCount = this.tempUpgradeCount;
    ...
    public void update()
    {
        ...
        {
            if ((cost == -1) && player.hasRelic(ChemicalX.ID))
            {
    ...
    private void renderTint(SpriteBatch sb)
    {
        if (!Settings.hideCards)
        {
            ...
            if (cardBgImg != null)
            {
                this.renderHelper(sb, color, cardBgImg, this.current_x, this.current_y);
            }
            else
            {
                this.renderHelper(sb, color, this.getCardBg(), this.current_x - 256.0F, this.current_y - 256.0F);
            }
        }
    ...
    private void renderDescription(SpriteBatch sb)
    {
        if (this.isSeen && !this.isLocked)
        {
            ...
            for (int i = 0; i < this.description.size(); ++i)
            {
                ...
                String[] var7 = (description.get(i)).getCachedTokenizedText();
                ...
                for (int var9 = 0; var9 < var8; ++var9)
                {
                    ...
                    //render the starting character and then remove it so it doesn't interfere w/ special characters
                    if (firstChar == '(')
                    {
                        gl.setText(font, "" + firstChar);
                        FontHelper.renderRotatedText(sb, font, "" + firstChar, this.current_x, this.current_y, start_x - this.current_x + gl.width / 2.0F, (float) i * 1.45F * -font.getCapHeight() + draw_y - this.current_y + -6.0F, this.angle, true, this.textColor);
                        start_x += gl.width;
                        
                        tmp = tmp.substring(1);
                    }
                    
                    //3 here is firstChar + at least 1 other character + " " at the end
                    if (tmp.length() >= 3 &&
                                (firstChar == '*' || firstChar == '#'))
                    {
                        //remove firstChar and the " " at the end
                        tmp = tmp.substring(1, tmp.length() - 1);
                        String punctuation = "";
    
                        //this should handle any amount of punctuation at the end. ex: "blah.)" -> punctuation = ".)"
                        ...
                        //assume * was first character -> gold
                        Color color = goldColor;
                        if (firstChar == '#')
                        {
                            char colorChar = tmp.charAt(0);
                            tmp = tmp.substring(1);
                            
                            color = BcUtility.getStringColor(colorChar);
                        }
                        
                        gl.setText(font, tmp);
                        FontHelper.renderRotatedText(sb, font, tmp, this.current_x, this.current_y, start_x - this.current_x + gl.width / 2.0F, (float) i * 1.45F * -font.getCapHeight() + draw_y - this.current_y + -6.0F, this.angle, true, color);
    ...
    private void renderTitle(SpriteBatch sb)
    {
        ...
        {
            FontHelper.cardTitleFont.getData().setScale(this.drawScale * titleRenderScale);
            Color textColor = getTitleColor();
            textColor.a = renderColor.a;
            FontHelper.renderRotatedText2(sb, FontHelper.cardTitleFont, titleToRender, this.current_x, this.current_y, 0.0F, 175.0F * this.drawScale * Settings.scale, this.angle, textColor);
        }
    }
    ...
    private void renderEnergy(SpriteBatch sb)
    {
        if (this.cost > -2 && !this.darken && !this.isLocked && this.isSeen)
        {
            ...
            if (isDimmed)
            {
                costColor = BcUtility.dimTextColor.cpy();
            }
    ...
    public boolean freeToPlay()
    {
        ...
        else if (BcUtility.isPlayerInCombat())
        {
            AbstractPlayer player = AbstractDungeon.player;
            if (this.type == AbstractCard.CardType.ATTACK)
            {
                //old swivel
                if (player.hasPower(FreeAttackPower.POWER_ID))
                {
                    return true;
                }
                else if (player.hasPower(SwivelPower.POWER_ID)) //new swivel
                {
                    SwivelPower swivel = (SwivelPower) player.getPower(SwivelPower.POWER_ID);
                    return swivel.shouldCardBeFree(this);
                }
            }
        }
    ...
    private void updateColor()
    {
        ...
        float a = renderColor.a;
        if (isDimmed)
        {
            renderColor = BcUtility.dimCardColor.cpy();
        }
        else
        {
    ...
    public Color getTitleColor()
    {
        ...
        if (isDimmed)
        {
            textColor = BcUtility.dimTextColor;
        }
        else if (isEthereal)
        {
    ...
    static
    {
        ...
        DESC_OFFSET_Y = Settings.BIG_TEXT_MODE ? IMG_HEIGHT * 0.23F : IMG_HEIGHT * 0.24F;
        DESC_BOX_WIDTH = IMG_WIDTH * 0.85f; //Settings.BIG_TEXT_MODE ? IMG_WIDTH * 0.95F : IMG_WIDTH * 0.79F;
        CN_DESC_BOX_WIDTH = Settings.BIG_TEXT_MODE ? IMG_WIDTH * 0.87F : IMG_WIDTH * 0.72F;
        TITLE_BOX_WIDTH = IMG_WIDTH * 0.68F;
        TITLE_BOX_WIDTH_NO_COST = IMG_WIDTH * 0.8F;
