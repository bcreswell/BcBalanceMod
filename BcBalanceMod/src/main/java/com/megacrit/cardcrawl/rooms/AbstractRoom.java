                        ...
                        if (this instanceof MonsterRoomBoss && !AbstractDungeon.loading_post_combat)
                        {
                            if (!CardCrawlGame.loadingSave)
                            {
                                ...
                                else
                                {
                                    ...
                                    //BC: replaced "poor bosses" with a different effect
...
//                                    }
    ...
    public void endBattle()
    {
        ...
        int attackCount = 0;
        int skillCount = 0;
        Iterator var3 = AbstractDungeon.actionManager.cardsPlayedThisCombat.iterator();
    ...
    public void addGoldToRewards(int gold)
    {
        ...
        } while (i.type != RewardItem.RewardType.GOLD);
    ...
    public void addStolenGoldToRewards(int gold)
    {
        ...
        do
        {
            ...
            i = (RewardItem) var2.next();
        } while (i.type != RewardItem.RewardType.STOLEN_GOLD);
    ...
    public boolean isBattleEnding()
    {
        ...
        {
            return this.monsters != null ? this.monsters.areMonstersBasicallyDead() : false;
        }
    }
        ...
        if (Settings.isControllerMode)
        {
            if (AbstractDungeon.getCurrRoom().event != null && AbstractDungeon.getCurrRoom().phase != AbstractRoom.RoomPhase.COMBAT && !AbstractDungeon.topPanel.selectPotionMode && AbstractDungeon.topPanel.potionUi.isHidden && !AbstractDungeon.topPanel.potionUi.targetMode && !AbstractDungeon.player.viewingRelics)
            {
                ...
                else if (!this.event.imageEventText.optionList.isEmpty())
                {
                    ...
                    if (!anyHovered)
                    {
                        Gdx.input.setCursorPosition((int) ((LargeDialogOptionButton) this.event.imageEventText.optionList.get(0)).hb.cX, Settings.HEIGHT - (int) ((LargeDialogOptionButton) this.event.imageEventText.optionList.get(0)).hb.cY);
                    }
                    else if (!CInputActionSet.down.isJustPressed() && !CInputActionSet.altDown.isJustPressed())
                    {
                        if (CInputActionSet.up.isJustPressed() || CInputActionSet.altUp.isJustPressed())
                        {
                            ...
                            Gdx.input.setCursorPosition((int) ((LargeDialogOptionButton) this.event.imageEventText.optionList.get(index)).hb.cX, Settings.HEIGHT - (int) ((LargeDialogOptionButton) this.event.imageEventText.optionList.get(index)).hb.cY);
                        }
                    }
                    else
                    {
    ...
    public void dispose()
    {
        ...
        {
            Iterator var1 = this.monsters.monsters.iterator();
