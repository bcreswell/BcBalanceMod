...
public class AwakenedOne extends AbstractMonster {
    ...
    private static final int SS_AMT = 3; //4;
        ...
        switch (this.nextMove) {
            ...
            case 8:
                ...
                for(i = 0; i < 3; ++i) {
                    ...
                    AbstractDungeon.actionManager.addToBottom(new DamageAction(AbstractDungeon.player, (DamageInfo)this.damage.get(4), AttackEffect.FIRE, true));
                }
        }
    ...
    protected void getMove(int num) {
        if (this.form1) {
            if (this.firstTurn) {
                this.setMove((byte)1, Intent.ATTACK, 20);
                this.firstTurn = false;
                return;
            }
            ...
            if (num < 25) {
                if (!this.lastMove((byte)2)) {
                    this.setMove(SS_NAME, (byte)2, Intent.ATTACK, 6, SS_AMT, true);
                } else {
                    this.setMove((byte)1, Intent.ATTACK, 20);
                }
            ...
            if (num < 50) {
                if (!this.lastTwoMoves((byte)6)) {
                    this.setMove(SLUDGE_NAME, (byte)6, Intent.ATTACK_DEBUFF, 18);
                } else {
                    this.setMove((byte)8, Intent.ATTACK, 10, 3, true);
                }
        ...
        if (this.currentHealth <= 0 && !this.halfDead) {
            ...
            while(true) {
                do {
                    ...
                    p = (AbstractPower)s.next();
                } while(p.type != PowerType.DEBUFF && !p.ID.equals("Curiosity") && !p.ID.equals("Unawakened") && !p.ID.equals("Shackled"));
