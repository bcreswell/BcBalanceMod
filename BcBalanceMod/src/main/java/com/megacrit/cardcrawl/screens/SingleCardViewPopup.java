...
public class SingleCardViewPopup
{
    ...
    public void open(AbstractCard card, CardGroup group)
    {
        ...
        isViewingUpgrade = card.upgraded;
    ...
    private void loadPortraitImg()
    {
        ...
        {
            this.portraitImg = getTexture("images/1024Portraits/" + this.card.assetUrl + ".png");
            if (this.portraitImg == null)
            {
                this.portraitImg = getTexture("images/1024PortraitsBeta/" + this.card.assetUrl + ".png");
            }
        }
        else
        {
    ...
    public void render(SpriteBatch sb)
    {
        ...
        {
            copy = this.card.makeStatEquivalentCopy();
            this.card = this.card.makeCopy(); //unupgraded
        }
...
label39:
    ...
    private void renderDescriptionCN(SpriteBatch sb)
    {
        if (!this.card.isLocked && this.card.isSeen)
        {
            ...
            for (int i = 0; i < this.card.description.size(); ++i)
            {
                ...
                String[] var8 = ((DescriptionLine) this.card.description.get(i)).getCachedTokenizedTextCN();
    ...
    private void renderDescription(SpriteBatch sb)
    {
        ...
        {
            BitmapFont font = FontHelper.SCP_cardDescFont;
            float draw_y = this.current_y + 88.0F * Settings.scale;
            draw_y += (float) this.card.description.size() * font.getCapHeight() * 0.775F - font.getCapHeight() * 0.375F;
            float spacing = 1.53F * -font.getCapHeight() / Settings.scale / this.drawScale;
            GlyphLayout gl = new GlyphLayout();
            ...
            for (int i = 0; i < this.card.description.size(); ++i)
            {
                ...
                for (int var10 = 0; var10 < var9; ++var10)
                {
                                ...
                                (firstChar == '*' || firstChar == '#'))
                    {
                        tmp = tmp.substring(1);
                        String punctuation = "";
                        char lastChar = tmp.charAt(tmp.length() - 2);
                        if (tmp.length() > 1 &&
                                    lastChar != '+' &&
                                    !Character.isLetterOrDigit(lastChar))
                        {
                            punctuation = punctuation + lastChar;
                            tmp = tmp.substring(0, tmp.length() - 2);
                            punctuation = punctuation + ' ';
                        }
                        ...
                        gl.setText(font, tmp);
                        FontHelper.renderRotatedText(sb, font, tmp, this.current_x, this.current_y, start_x - this.current_x + gl.width / 2.0F, (float) i * 1.53F * -font.getCapHeight() + draw_y - this.current_y + -12.0F, 0, true, color);
                        start_x = (float) Math.round(start_x + gl.width);
                        gl.setText(font, punctuation);
                        FontHelper.renderRotatedText(sb, font, punctuation, this.current_x, this.current_y, start_x - this.current_x + gl.width / 2.0F, (float) i * 1.53F * -font.getCapHeight() + draw_y - this.current_y + -12.0F, 0, true, Settings.CREAM_COLOR);
                        gl.setText(font, punctuation);
                        start_x += gl.width;
                    }
    ...
    private void renderTitle(SpriteBatch sb)
    {
        if (this.card.isLocked)
        {
            FontHelper.SCP_cardTitleFont_small.getData().setScale(1);
        ...
        {
            FontHelper.SCP_cardTitleFont_small.getData().setScale(card.titleRenderScale);
            
            FontHelper.renderFontCentered(
                    sb,
                    FontHelper.SCP_cardTitleFont_small,
                    card.titleToRender,
                    (float) Settings.WIDTH / 2.0F + card.titleXOffset,
                    (float) Settings.HEIGHT / 2.0F + 338.0F * Settings.scale,
                    card.getTitleColor());
        }
        else
        {
