...
public class VictoryScreen extends GameOverScreen {
    ...
    private void createGameOverStats() {
        this.stats.clear();
        this.stats.add(new GameOverStat(TEXT[1] + " (" + AbstractDungeon.floorNum + ")", (String)null, Integer.toString(floorPoints)));
        this.stats.add(new GameOverStat(TEXT[8] + " (" + CardCrawlGame.monstersSlain + ")", (String)null, Integer.toString(monsterPoints)));
        ...
        this.stats.add(new GameOverStat(TEXT[4], (String)null, Integer.toString(this.score)));
    }
    
    protected void submitVictoryMetrics() {
        Metrics metrics = new Metrics();
        metrics.gatherAllDataAndSave(false, true, (MonsterGroup)null);
        if (Settings.UPLOAD_DATA) {
            metrics.setValues(false, true, (MonsterGroup)null, Metrics.MetricRequestType.UPLOAD_METRICS);
    ...
    private void updateAscensionAndBetaArtProgress() {
        if (AbstractDungeon.isAscensionMode && !Settings.seedSet && !Settings.isTrial && AbstractDungeon.ascensionLevel < 20 && StatsScreen.isPlayingHighestAscension(AbstractDungeon.player.getPrefs())) {
           ...
           {
               //bc: changed it so you don't unlock the next ascension until you beat the heart
               StatsScreen.incrementAscension(AbstractDungeon.player.getCharStat());
    ...
    private void updateVfx() {
        switch(AbstractDungeon.player.chosenClass) {
            ...
            case WATCHER:
                ...
                Iterator var3 = this.effect.iterator();
